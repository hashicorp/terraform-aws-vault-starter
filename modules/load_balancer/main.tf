...
resource "aws_security_group_rule" "vault_lb_inbound_nodes" {
  count                    = var.lb_type == "application" ? 1 : 0
  description              = "Allow inbound traffic from vault nodes on port 8200"
...
resource "aws_security_group_rule" "vault_lb_inbound_extra" {
